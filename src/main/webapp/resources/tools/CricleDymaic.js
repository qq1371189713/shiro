...
function CircleShow(radius,level,point,color){
	if(!window.map || !window.AMap|| !window.AMap.Circle){
		return undefined;
	}
	this.radius = radius; 
    ...
    this.circle0 = new AMap.Circle(this.point,this.radius,{
	    fillColor:"white", 
	    fillOpacity: 0.2,
	    strokeWeight: 1 ,
	    strokeColor:"white",
	...
	map.add(this.circle0);
    
	//按层数循环构造覆盖物，并加在图片上
    this.circles = new Array();
    for(let i=1; i< this.level; i++){
    	let circle = new AMap.Circle(this.point,0,{
...
CircleShow.prototype.start = function (distance,t0){
    ...
    function animateStart(startTime,circle,index){
    	...
    	if(time<0){
    		circle.radius = 0;						//半径
        	circle.fillOpacity(_self.color.fillColor);	//透明度
