...
//						createInfo(this).open(map, [marker.lng, marker.lat]);
						createPileInfo(this).open(map, [marker.lng, marker.lat]);
					});
				}
				setTimeout(function(){
					var markers = markerMap.values();
...
//	marker1.jeevesType = marker.jeevesType;
	...
	content += "预警时间:2019-03-27 12:23:43<br><br>";
	content += "占道路段: <span id='location'></span><br>";
	...
	content += "设备编号:" + data.deviceNum + "<br>";
